 ...
 * @author zied
 */
@Component("bilanAnnuelController")
@Scope("view")
public class BilanAnnuelController extends BasePage implements Serializable{
    private List<BilanAV> bilanAV=new ArrayList<>();
    private List<BeneficeClient> bilanBen=new ArrayList<>();
    private int anneeBilan;
    ...
    public void search(){
        ...
        for(OpVente opV:opVenteManager.getByYear(anneeBilan-1900)){
        ...
        for(OpAchat opA:opAchatManager.getByYear(anneeBilan-1900)){
        ...
        for(Client c:clientManager.getAll()){
            ...
            bc.setClient(c);
            float benefice=0;
            List<OpVente> opVs=opVenteManager.getByYearByClient(anneeBilan-1900, c);
    ...
    public void init(){
        bilanAV=new ArrayList<>();
        anneeBilan=Calendar.getInstance().get(Calendar.YEAR);
    ...
    public int getAnneeBilan() {
        return anneeBilan;
    }

    public void setAnneeBilan(int anneeBilan) {
