 ...
 * @author zied
 */
@Component("bilanMensuelController")
@Scope("view")
public class BilanMensuelController extends BasePage implements Serializable{
    ...
    public void search(){
        ...
        for(OpVente opV:opVenteManager.getByMonth(dateBilan)){
        ...
        for(OpAchat opA:opAchatManager.getByMonth(dateBilan)){
        ...
        for(Client c:clientManager.getAll()){
            ...
            bc.setClient(c);
            float benefice=0;
            List<OpVente> opVs=opVenteManager.getByMonthByClient(dateBilan, c);
